...
add_shortcode('join', 'update_team_page');
...
function update_team_page()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        // Check if the user's username already exists in the team table
        $existing_user_team = $wpdb->get_var($wpdb->prepare("SELECT team FROM $table_name WHERE username = %s", $username));

        if ($existing_user_team) {
            // The user is already in a team, show an alert message
            echo '<script>alert("You are already in a team.");</script>';
            $redirect_url = site_url('/changeteam/'); // Change this to an appropriate URL
            ?>
            <script>
        ...
        // If the user's username is not in the table, proceed with the join operation
        ...
        if ($result) {
            $redirect_url = site_url('/teamspage/'); // Change this to an appropriate URL
            ?>
            <script>
                window.location.href = "<?php echo $redirect_url; ?>";
            </script>
        ...
        <div class="row">
            <div class="col-sm-12">
                <form method="POST" action="">
                    <!-- select team -->
                    <label for="team">Select your team and join. Please refer to the list to check what team you are in :) <a id="teams-list" href="https://stellenbosch-my.sharepoint.com/:x:/g/personal/shanes_sun_ac_za/ERTXx1dMoZZJuVy8HPnas7kBkS2hXgWEmYVu5wHDUTM3NQ?e=dhyomB" target="_blank">TEAMS LIST<a></label><br>
